...
class Decompressor
{
  ...
  public:
                    ...
                    std::size_t size) const
    {
        ...
        constexpr int window_bits = 15; // auto with windowbits of 15

#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wold-style-cast"
        int init_status = inflateInit2(&inflate_s, window_bits);
        if (init_status != Z_OK)
        {
            throw std::runtime_error("inflate init failed with code " + std::to_string(init_status));
        }
#pragma GCC diagnostic pop
        inflate_s.next_in = reinterpret_cast<const unsigned char*>(data);
        ...
        do
        {
            ...
            ret = inflate(&inflate_s, Z_NO_FLUSH);
